    ...
    try {
        ...
        if (globleFilters) {
            matchQuery["$or"] = [
                ...
                { sku: { $regex: globleFilters, $options: 'i' } },
                { size: { $regex: globleFilters, $options: 'i' } },
                { color: { $regex: globleFilters, $options: 'i' } },
            ]
        }
        ...
        filters.forEach(filter => {
            ...
            if (id === "product") {
                matchQuery["productData.name"] = { $regex: value, $options: 'i' }; // ✅ name field filter
        ...
        const productData = await ProductVariantModel.aggregate(aggregatePipeline);
        const totalProducts = await ProductVariantModel.countDocuments(matchQuery);
