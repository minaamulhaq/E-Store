...
const AddProduct = () => {
    ...
    useEffect(() => {
        if (getCategory?.success) {
            const options = getCategory.data.map((category) => ({
                value: category._id,
                label: category.name,
            }));
    ...
    const HandleSubmit = async (value) => {
        ...
        setLoading(true);
        try {
            const { data: response } = await axios.post('/api/product/create', { ...value, media: mediaIds });
            if (response.success) {
                form.reset();
                setSelectedMedia([]);
                ...
                <CardContent className={`pb-5`}>
                    <Form {...form}>
                        <form onSubmit={form.handleSubmit(HandleSubmit)} >
                            ...
                            <div className="grid md:grid-cols-2 grid-cols-1 gap-5 mb-3">
                                <div>
                                    <FormField
