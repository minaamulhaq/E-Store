...
export const reviewSchema = z.object({
    ...
    rating: z.union([
        z.number().positive("Review must be a positive number"),
        z.string().transform((val) => Number(val)).refine((val) => !isNaN(val) && val > 0, {
            message: "Review must be a positive number",
        }),
    ]),
    title: z.string().min(3, "Title is required"),
...
export const coupanValidation = z.object({
    code: z
        .string().min(3, "Code must be at least 3 characters long")
        .max(50, "Code must be at most 50 characters long")
        .regex(/^[A-Za-z0-9_-]+$/, "Code can only contain letters, numbers, underscores, and hyphens"),
    minShoppingAmount: z.union([
        z.number({ invalid_type_error: "Selling Price must be a number" })
...
export const orderSchema = z.object({
    ...
    ordernote: z.string().min(3, "Order Note is required"),
});